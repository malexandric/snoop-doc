...
- **Auth: restricted API key**, stored in `~/.snoop-doc/settings.json` under `stripe_api_key`. Same model as the Anthropic key — per-user, never committed. Users create a restricted key in Stripe Dashboard with read-only access to Customers, Subscriptions, Invoices, Charges, Refunds, Disputes, Payouts, Products, Prices, Coupons, Promotion Codes. Much simpler than the Google Sheets OAuth dance because there's no per-user identity to track on Stripe's side; the API key IS the identity.
- **Twelve CSVs** (the data model):
  - Core data (high-volume, incremental sync):
    ...
    - `stripe_refunds.csv` — one row per refund (with reason, amount, status, charge_id).
    - `stripe_disputes.csv` — one row per dispute / chargeback (status, reason, evidence due-by, network reason code).
...
- [x] **Stripe live sync.** ✓ Restricted API key, twelve CSVs (customers / subs / sub_items / invoices / invoice_line_items / charges / refunds / disputes / payouts / products / prices / coupons / promotion_codes), initial full pull + incremental refresh via `created[gte]`. Sync registry + same per-row UI as Google Sheets. Sync-on-app-start toggle gated to "only after first manual sync" so the 25-min initial pull never surprises anyone at launch.
...
- [ ] **Save chat history (saved conversations).** A fourth Saved Items type alongside charts / tables / reports — the whole conversation thread (every user + assistant turn, plus the figures and tables produced during it, plus the table-scope picker state at save time). Distinct from reports because a saved conversation is **resumable**: clicking it loads the messages back into `st.session_state.messages` and you continue from where you left off; reports are static artefacts. Storage: `saved/conversations/{id}.json` + a `.meta.json` with name + created + last-resumed timestamp + a short summary (first user question, or an AI-generated one-line title). Actions in the Saved Items view: View (read-only preview), **Resume** (loads into chat + switches to the chat view), Rename, Delete. Manual save only — a "Save conversation" button in the sidebar under Clear chat, visible only on the chat view when there are messages. No auto-save: avoids cluttering the list with throwaway sessions and forces a deliberate "this was worth keeping" action. Figures + tables are stored alongside the message JSON (same pattern as the in-memory `st.session_state.figures` / `tables` keyed by `tool_use_id`) so resuming a conversation re-renders correctly.
