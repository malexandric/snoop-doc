"""
Saved Items: storage + UI for charts, tables, (and later) reports the user
keeps from their chat sessions.
...
ITEM_TYPES = ("charts", "tables", "reports")
EXT_BY_TYPE = {
    "charts": "json",
    "tables": "csv",
    "reports": "html",
...
def render() -> None:
    ...
    sub_charts, sub_tables, sub_reports, sub_trash = st.tabs(
        ["Charts", "Tables", "Reports", "Trash"]
    )
...
def _render_card(item_type: str, item: dict) -> None:
    ...
    with st.container(border=True):
        ...
        n_actions = 4 if item_type in ("tables", "reports") else 3
